...
const CashFlowPlanPage: React.FC = () => {
  ...
  const [selectedPlan, setSelectedPlan] = useState('Plan Tresorerie 2024');
  ...
  const handleExport = () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Plan Tresorerie');
    ...
    <div className="space-y-6 mb-12">
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-[32px] border border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden">
        <div className="px-8 py-8 border-b border-zinc-200 dark:border-zinc-800 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 bg-zinc-50/30">
          <div>
            <h2 className="text-3xl font-black text-zinc-900 dark:text-white tracking-tight">Plan de Tresorerie Consolide</h2>
            <p className="text-zinc-500 font-medium mt-1">Reporting multi-entites CIE - Analyse du BFR en temps reel.</p>
          </div>

          {/* Period & Plan selectors + actions */}
          <div className="flex flex-wrap gap-3 items-center">
            ...
            <button
              ...
              className={`h-10 px-6 rounded-xl text-xs font-black border transition-all flex items-center justify-center gap-2 ${
                isExporting
                  ? 'bg-zinc-100 text-zinc-400 border-zinc-200 cursor-not-allowed'
                  : 'bg-white dark:bg-zinc-800 border-zinc-200 dark:border-zinc-700 hover:border-[#e65000] text-zinc-700 dark:text-zinc-200 hover:text-[#e65000]'
              }`}
            >
              <span className={`material-symbols-outlined text-lg ${isExporting ? 'animate-spin' : ''}`}>
                {isExporting ? 'sync' : 'download'}
              </span>
              {isExporting ? 'TRAITEMENT...' : 'EXPORTER EXCEL'}
            </button>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[900px]">
            <thead>
              <tr className="bg-zinc-50 dark:bg-zinc-800/50 text-zinc-500 text-[10px] font-black uppercase tracking-widest border-b border-zinc-200 dark:border-zinc-800">
            ...
            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
              ...
              <tr className="bg-zinc-900 text-white font-black text-sm">
                <td className="px-8 py-6 uppercase tracking-wider">Flux Net de Tresorerie</td>
                {allMonths.map((m, i) =>
                  visibleMonths.includes(m) ? (
                    <td key={m} className={`px-8 py-6 text-center ${m === 'Mars' ? 'text-green-400 bg-white/5 font-black text-base' : 'text-zinc-400'}`}>
